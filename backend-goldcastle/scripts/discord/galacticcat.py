...
import asyncio
import os
from galactic_cat_response import generate_response, generate_compliment
...
@bot.tree.command(name='alphcatverify', description='Verify your account')
async def verify(interaction: discord.Interaction):
    ...
    if interaction.channel.id == verification_channel_id or verification_channel_id == "":
        ...
        redirect_url = f'http://sign.galactics.org/alphcat/{discord_id}'
        
        embed = discord.Embed(title='Verification', description='Click here to verify your token and NFT ownership:', color=0x00ff00)
        embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/1226789689974067203/1228722042979749958/galacticcatclose.png?ex=662d1393&is=661a9e93&hm=290a3ec8607f77424e491b7b176b98ec641d02ebaf83adb1b5ff313076ee0edc&')
...
@bot.tree.command(name='alphcatconfigure', description='Configure guild settings')
@commands.has_permissions(administrator=True)
async def configure(interaction: discord.Interaction):

    if interaction.user.guild_permissions.administrator:
        guild_id = interaction.guild.id
        guild_config_path = f"alphcat/guild_configs/guild_{guild_id}.json"
        template_path = "alphcat/guild_configs/template.json"
    ...
    """)
                ...
                with open(f'alphcat/guild_configs/guild_{interaction.guild.id}.json', 'r') as f:
                    config = json.load(f)
                if token_id not in config['TOKEN_ROLES']:
                    config['TOKEN_ROLES'][token_id] = []
                config['TOKEN_ROLES'][token_id].extend(token_roles)
                with open(f'alphcat/guild_configs/guild_{interaction.guild.id}.json', 'w') as f:
...
@bot.tree.command(name='alphcatconfiguration', description='View and manage existing rules')
...
@bot.tree.command(name='alphcatroles', description='Checking roles')
async def alphcatroles(interaction: discord.Interaction):
...
async def update_roles():
    while True:
        ...
        await asyncio.sleep(32400)  # Wait for 9 hours (32400 seconds)
...
@bot.tree.command(name='alphcatcompliment', description='Let ALPH cat compliment a random member')
async def alphcatcompliment(interaction: discord.Interaction):
...
@bot.tree.command(name='alphcatcomplimentrelay', description='Let ALPH cat compliment a member on a specific subject')
async def alphcatcomplimentrelay(interaction: discord.Interaction, member: discord.Member, subject: str):
...
def load_config():
    try:
        with open('alphcat/guild_configs/guilds_alphcat.json', 'r') as file:
            return json.load(file)
    except FileNotFoundError:
        return {"configuration": []}

def save_config(config):
    with open('alphcat/guild_configs/guilds_alphcat.json', 'w') as file:
        json.dump(config, file, indent=4)


class SettingsModal(ui.Modal, title="alphcat Settings"):
...
@bot.tree.command(name='alphcatsettings', description='Manage the settings for ALPH cat')
async def alphcatsettings(interaction: discord.Interaction):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("You are not an admin :clown:", ephemeral=True)
        return
...
@bot.tree.command(name='alphcatshowcommands', description='Show all available commands')
async def alphcatshowcommands(interaction: discord.Interaction):
    roles_message = f"""
/alphcatverify = Sign to prove NFT / Token ownership
/alphcatroles = Let ALPH cat check your roles :nail_care:
/alphcatshowcommands = Show all available commands
/alphcatcompliment = Let ALPH cat make a random compliment
/alphcatcomplimentrelay = Let ALPH cat compliment a member on a specific subject

:crown:
For the admin:
/alphcatsettings = Configure ALPH cat's settings for your server
/alphcatconfigure = Add new rules for roles ALPH cat can hand out
/alphcatconfiguration = Edit or delete rules
/alphcatreset = Reset the configuration
:crown:
"""
    await interaction.response.send_message(roles_message)


@bot.tree.command(name='alphcatshowroles', description='Show all available commands')
async def alphcatshowcommands(interaction: discord.Interaction):
    guild_id = interaction.guild.id
    ...
    try:
        ...
        file_path = os.path.join(script_dir, f"alphcat/guild_configs/guild_{guild_id}.json")
...
@bot.tree.command(name='alphcatreset', description='Reset the configuration')
async def alphcatreset(interaction: discord.Interaction):
    if interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("This will reset the configuration and remove all roles that can currently be assigned by ALPH cat from members. Are you sure you want to reset the configuration? (y/n)")
        try:
            ...
            if msg.content.lower() == 'y':
                ...
                async def button_callback(button_interaction: discord.Interaction):
                    ...
                    # Construct the relative path to the JSON file
                    config_path = os.path.join(script_dir, f"alphcat/guild_configs/guild_{guild_id}.json")
                    template_path = os.path.join(script_dir, f"alphcat/guild_configs/template.json")
...
botname = "ALPH Cat"
bot_token = os.environ.get('GALACTIC_CAT_DISCORD_BOT_TOKEN')
if bot_token:
    bot.run(bot_token)
